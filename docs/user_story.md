# Kotlin 증권: 자산 가격 추적 API

## 프롤로그

Kotlin 증권은 고객이 여러 시장의 투자 자산 가격을 손쉽게 추적할 수 있는 서비스를 준비하고 있습니다.

처음에는 단순했습니다.

> "마켓을 만들고, 자산을 등록하고, 가격을 저장하자."

하지만 실제 금융 서비스에서는 단순한 CRUD만으로는 부족했습니다.

자산은 반드시 특정 마켓에 속해야 했고, 가격 이력은 특정 자산에 연결되어야 했습니다.
시장마다 통화와 시간대가 달랐고, 자산은 거래 중지나 상장 폐지 상태가 될 수도 있었습니다.

가격 데이터 역시 단순히 "언제 얼마였는가"만 저장해서는 부족했습니다.
외부 시세 제공자마다 데이터가 들어오는 시점이 다를 수 있고, 실제 시장 시각과 서버가 데이터를 받은 시각도 구분되어야 했습니다.

고객은 가격 목록뿐 아니라 최고가, 최저가, 평균가 같은 분석 정보를 원했고, 인기 자산에는 동시에 많은 가격 업데이트가 들어오기 시작했습니다.

## 도메인 모델

### Market

마켓은 자산이 거래되는 시장입니다.

예: KOSPI, NASDAQ, CRYPTO

| 컬럼 | 설명 |
| --- | --- |
| `id` | 마켓 ID |
| `name` | 마켓 이름 |
| `timezone` | 마켓 기준 시간대 |
| `createdAt` | 생성 시각 |
| `updatedAt` | 수정 시각 |

#### 제약 조건

- `name`은 중복될 수 없습니다.
- `timezone`은 필수입니다.
- `createdAt`, `updatedAt`은 서버에서 기록합니다.

### Asset

자산은 반드시 하나의 마켓에 속합니다.

예:

- KOSPI - 삼성전자
- NASDAQ - AAPL
- CRYPTO - BTC

| 컬럼 | 설명 |
| --- | --- |
| `id` | 자산 ID |
| `market` | 소속 마켓 |
| `symbol` | 자산 심볼 |
| `name` | 자산 이름 |
| `status` | 자산 상태 |
| `currency` | 가격 통화 |
| `createdAt` | 생성 시각 |
| `updatedAt` | 수정 시각 |

#### AssetStatus 예시

- `ACTIVE`
- `INACTIVE`
- `DELISTED`

#### Currency 예시

- `KRW`
- `USD`
- `BTC`
- `USDT`

#### 제약 조건

- 자산은 반드시 마켓에 속해야 합니다.
- 하나의 마켓 안에서 같은 `symbol`은 중복될 수 없습니다.
- Unique index: `(market_id, symbol)`

### 최신 가격 정보

가격 이력만 append 한다면 동시성 문제는 비교적 단순합니다.
하지만 화면이나 외부 연동에서 자산의 최신 가격을 빠르게 조회하려면 최신 가격 정보를 별도로 관리해야 합니다.

아래 방식 중 하나를 선택해 구현합니다.

#### 선택지 A. Asset에 최신 가격 컬럼 추가

- `Asset.currentPrice`
- `Asset.lastPriceAt`
- `Asset.lastPriceSource`
- `Asset.version`

#### 선택지 B. LatestPrice 테이블 분리

| 컬럼 | 설명 |
| --- | --- |
| `asset` | 대상 자산 |
| `price` | 최신 가격 |
| `timestamp` | 최신 가격의 시장 기준 시각 |
| `source` | 최신 가격 출처 |
| `version` | 낙관적 락 버전 |
| `updatedAt` | 수정 시각 |

최신 가격을 갱신하는 로직에는 동시성 제어가 적용되어야 합니다.

> 최신 가격은 자산 마스터 정보와 변경 빈도와 갱신 주체가 다르다.
> 가격은 SYSTEM에 의해 자주 갱신되고 낙관적 락 대상이 되지만, 자산의 이름, 상태, 통화 같은 마스터 정보는 ADMIN에 의해 상대적으로 드물게 변경된다.
> 최신 가격을 Asset에 포함하면 가격 갱신마다 assets row의 version이 증가하여 자산 마스터 수정과 불필요한 락 충돌이 발생할 수 있다.
> 따라서 가격 갱신의 동시성 제어 범위를 최신 가격 row로 좁히기 위해 LatestPrice를 별도 엔티티로 분리하는 것을 선택한다.

### PriceHistory

가격 이력은 특정 자산의 특정 시점 가격을 의미합니다.

| 컬럼 | 설명 |
| --- | --- |
| `id` | 가격 이력 ID |
| `asset` | 대상 자산 |
| `price` | 가격 |
| `timestamp` | 실제 시장 기준 시각 |
| `source` | 가격 데이터 출처 |
| `receivedAt` | 서버가 데이터를 수신한 시각 |
| `createdAt` | 생성 시각 |
| `updatedAt` | 수정 시각 |

`timestamp`와 `receivedAt`은 서로 다릅니다.

| 컬럼 | 의미 |
| --- | --- |
| `timestamp` | 실제 시장에서 가격이 발생한 시각 |
| `receivedAt` | Kotlin 증권 서버가 데이터를 받은 시각 |

예를 들어 외부 시세 제공자가 10:00 가격을 10:00:03에 보냈다면 다음처럼 저장될 수 있습니다.

```json
{
  "price": 72000,
  "timestamp": "2026-05-03T10:00:00",
  "receivedAt": "2026-05-03T10:00:03"
}
```

#### 제약 조건

- 가격은 0보다 커야 합니다.
- 가격 이력은 반드시 자산에 속해야 합니다.
- `price > 0` 제약을 DB에도 둡니다.
- `createdAt`, `updatedAt`은 서버에서 기록합니다.

## 에러 응답 형식

모든 에러는 아래 형식을 따릅니다.

```json
{
  "code": "ASSET_NOT_FOUND",
  "message": "Asset not found."
}
```

## Chapter 1. 첫 번째 기능: 마켓을 만든다

서비스의 출발점은 마켓입니다.

Kotlin 증권은 KOSPI, NASDAQ, CRYPTO처럼 다양한 시장을 관리하려고 합니다.
관리자는 새로운 마켓을 등록할 수 있어야 하며, 같은 이름의 마켓이 중복으로 생성되어서는 안 됩니다.
사용자와 운영 도구는 등록된 마켓을 목록으로 조회하거나, 특정 마켓을 ID 또는 이름으로 조회할 수 있어야 합니다.

### 구현할 API

`POST /markets`

`GET /markets`

`GET /markets/{marketId}`

`GET /markets/name/{name}`

### 생성 요청 예시

```json
{
  "name": "KOSPI",
  "timezone": "Asia/Seoul"
}
```

### 조회 요청 예시

목록 조회

`GET /markets`

ID 단건 조회

`GET /markets/1`

이름 단건 조회

`GET /markets/name/KOSPI`

### 요구사항

- 마켓 이름은 중복될 수 없습니다.
- 마켓 이름은 unique 하므로 이름으로 단건 조회할 수 있습니다.
- `timezone`은 필수입니다.
- 성공 시 `201 Created`를 반환합니다.
- 이미 존재하는 마켓이면 `409 Conflict`를 반환합니다.
- 목록 조회 시 `200 OK`와 마켓 목록을 반환합니다.
- ID 단건 조회 시 `200 OK`와 마켓 정보를 반환합니다.
- 이름 단건 조회 시 `200 OK`와 마켓 정보를 반환합니다.
- 존재하지 않는 마켓 ID 또는 이름이면 `404 Not Found`를 반환합니다.
- 중복 제약은 애플리케이션 레벨과 DB unique index 양쪽에서 처리합니다.

## Chapter 2. 마켓에는 자산이 속한다

고객이 실제로 추적하고 싶은 것은 자산입니다.

자산은 독립적으로 존재하지 않습니다.
반드시 하나의 마켓에 속해야 합니다.

### 구현할 API

`POST /markets/{marketId}/assets`

`GET /markets/{marketId}/assets`

`GET /markets/{marketId}/assets/{assetId}`

### 생성 요청 예시

```json
{
  "symbol": "005930",
  "name": "삼성전자",
  "currency": "KRW"
}
```

### 조회 요청 예시

자산 목록 조회

`GET /markets/1/assets`

자산 단건 조회

`GET /markets/1/assets/10`

### 요구사항

- 존재하지 않는 마켓에는 자산을 등록할 수 없습니다.
- 하나의 마켓 안에서 같은 `symbol`은 중복될 수 없습니다.
- 자산 생성 시 기본 상태는 `ACTIVE`입니다.
- `currency`는 필수입니다.
- 성공 시 `201 Created`를 반환합니다.
- 존재하지 않는 마켓이면 `404 Not Found`를 반환합니다.
- 중복된 자산이면 `409 Conflict`를 반환합니다.
- 목록 조회 시 `200 OK`와 특정 마켓에 속한 자산 목록을 반환합니다.
- 단건 조회 시 `200 OK`와 자산 정보를 반환합니다.
- 존재하지 않는 자산이거나 `marketId`와 `assetId`의 관계가 맞지 않으면 `404 Not Found`를 반환합니다.
- 자산을 생성한 뒤 조회 API를 통해 등록된 내역을 확인할 수 있어야 합니다.
- 중복 제약은 애플리케이션 레벨과 DB unique index 양쪽에서 처리합니다.

## Chapter 3. 자산에는 상태가 있다

실제 금융 서비스에서 자산은 항상 거래 가능한 상태가 아닙니다.

어떤 자산은 정상 거래 중일 수 있고, 어떤 자산은 일시적으로 비활성화될 수 있으며, 어떤 자산은 상장 폐지될 수도 있습니다.

### 상태 예시

- `ACTIVE`
- `INACTIVE`
- `DELISTED`

### 요구사항

- 가격 등록은 `ACTIVE` 상태의 자산에만 가능합니다.
- `INACTIVE`, `DELISTED` 상태의 자산에는 가격을 등록할 수 없습니다.
- 상태 변경 API를 구현하거나, 구현하지 않는 경우 설계 설명에 상태 전환 방식을 포함합니다.

### 선택 구현 API

`PATCH /markets/{marketId}/assets/{assetId}/status`

### 요청 예시

```json
{
  "status": "INACTIVE"
}
```

## Chapter 4. 자산에는 가격 이력이 쌓인다

자산이 등록되었다면 이제 가격을 기록해야 합니다.

가격은 현재 값 하나만 중요한 것이 아닙니다.
언제 얼마였는지 계속 쌓여야 나중에 추이와 통계를 분석할 수 있습니다.

### 구현할 API

`POST /markets/{marketId}/assets/{assetId}/prices`

### 요청 예시

```json
{
  "price": 72000,
  "timestamp": "2026-05-03T10:00:00",
  "source": "SYSTEM_A"
}
```

### 요구사항

- 가격은 0보다 커야 합니다.
- `timestamp`는 필수입니다.
- `source`는 필수입니다.
- `receivedAt`은 서버에서 저장 시점에 기록합니다.
- 존재하지 않는 자산에는 가격을 저장할 수 없습니다.
- `marketId`와 `assetId`의 관계가 맞지 않으면 저장할 수 없습니다.
- 자산 상태가 `ACTIVE`가 아니면 가격을 저장할 수 없습니다.
- 가격 이력 저장과 최신 가격 갱신은 하나의 트랜잭션으로 처리합니다.
- 성공 시 `201 Created`를 반환합니다.
- 자산이 없거나 소속 관계가 맞지 않으면 `404 Not Found`를 반환합니다.
- 가격이 유효하지 않으면 `400 Bad Request`를 반환합니다.
- 자산 상태가 유효하지 않으면 `400 Bad Request`를 반환합니다.

## Chapter 5. 고객은 숫자가 아니라 인사이트를 원한다

고객은 단순한 가격 목록이 아니라 분석 정보를 원합니다.

> "이번 달 삼성전자의 최고가는 얼마였나요?"
> "최저가는요?"
> "평균 가격은 어느 정도였나요?"

모든 가격 데이터를 애플리케이션 메모리로 가져와 계산하는 방식은 데이터가 많아질수록 위험합니다.
따라서 DB 레벨의 집계 쿼리가 필요합니다.

### 구현할 API

`GET /markets/{marketId}/assets/{assetId}/prices/statistics?from=2026-05-01T00:00:00&to=2026-05-03T23:59:59`

### 응답 예시

```json
{
  "assetId": 1,
  "symbol": "005930",
  "currency": "KRW",
  "minPrice": 71000,
  "maxPrice": 73500,
  "averagePrice": 72250
}
```

### 요구사항

- 특정 기간의 최고가, 최저가, 평균가를 반환해야 합니다.
- DB 레벨 집계 쿼리를 사용해야 합니다.
- 존재하지 않는 자산이면 `404 Not Found`를 반환합니다.
- `marketId`와 `assetId`의 관계가 맞지 않으면 `404 Not Found`를 반환합니다.
- `from`은 `to`보다 이전이어야 합니다.
- `from`, `to`가 없거나 파싱할 수 없으면 `400 Bad Request`를 반환합니다.
- 날짜 범위가 유효하지 않으면 `400 Bad Request`를 반환합니다.

### 데이터가 없는 경우

기간 내 가격 데이터가 없다면 `200 OK`와 함께 집계 값을 `null`로 반환합니다.

```json
{
  "assetId": 1,
  "symbol": "005930",
  "currency": "KRW",
  "minPrice": null,
  "maxPrice": null,
  "averagePrice": null
}
```

## Chapter 6. 동시에 들어오는 가격 업데이트

서비스가 외부 시세 제공자와 연결되면서 문제가 생겼습니다.

같은 자산의 가격 데이터가 여러 경로에서 거의 동시에 들어오기 시작했습니다.

가격 이력만 단순히 append 한다면 동시성 문제는 비교적 적습니다.
하지만 최신 가격을 함께 갱신한다면 문제가 발생할 수 있습니다.

예를 들어 두 요청이 동시에 같은 자산의 최신 가격을 읽고, 각자 최신 가격을 갱신하려 한다면 더 늦게 도착한 오래된 가격이 최신 가격을 덮어쓸 수도 있습니다.

### 구현 또는 설계할 내용

- 가격 이력 저장과 최신 가격 갱신을 하나의 트랜잭션으로 처리합니다.
- 최신 가격 갱신 과정에서 동시성 문제가 발생할 수 있는 지점을 설명합니다.
- 낙관적 락 또는 비관적 락을 적용합니다.
- 낙관적 락을 사용하는 경우 최신 가격 정보를 관리하는 엔티티에 `@Version` 필드를 둡니다.
- 충돌 발생 시 `409 Conflict`를 반환합니다.
- 가능하다면 동시 요청 테스트를 작성합니다.

### 낙관적 락 예시

최신 가격 정보를 관리하는 엔티티에 `version` 컬럼을 둡니다.

- `Asset.version`
- `LatestPrice.version`

충돌이 발생하면 다음 응답을 반환합니다.

```json
{
  "code": "CONCURRENCY_ERROR",
  "message": "Price update conflict occurred. Please retry."
}
```

## Chapter 7. 충돌이 너무 많아지면 락도 병목이 된다

인기 자산에는 가격 업데이트가 매우 자주 들어옵니다.

낙관적 락을 적용했더라도 충돌이 계속 발생하면 요청들이 반복적으로 실패하고 재시도하게 됩니다.
이 재시도가 과도해지면 애플리케이션은 CPU와 DB 리소스를 낭비할 수 있습니다.

이제 문제는 "락을 걸었는가"가 아니라, "락 경합을 어떻게 줄였는가"입니다.

### 구현 또는 설계할 내용

다음 중 하나 이상을 선택해 해결합니다.

- 재시도 횟수 제한
- Exponential Backoff
- 비동기 큐 기반 가격 업데이트
- 자산 단위 직렬 처리
- 분산 락
- 마지막 가격만 반영하는 Coalescing 전략

### 권장 정책 예시

- 낙관적 락 충돌 시 최대 3회까지만 재시도합니다.
- 재시도 간격은 Exponential Backoff를 적용합니다.
- 예: 100ms, 200ms, 400ms
- 충돌이 계속 발생하면 `409 CONCURRENCY_ERROR`를 반환합니다.
- 고빈도 자산은 큐 기반으로 자산 단위 직렬 처리를 고려합니다.
- 같은 자산에 짧은 시간 동안 여러 가격이 들어오면 마지막 가격만 최신 가격에 반영하는 Coalescing 전략을 고려합니다.

## Chapter 8. 금융 데이터는 아무나 만질 수 없다

자산 가격 데이터는 서비스의 핵심 데이터입니다.

모든 사용자가 마켓을 만들거나 가격을 등록할 수 있어서는 안 됩니다.
일반 사용자는 조회만 가능해야 하고, 관리자나 시스템 권한을 가진 사용자만 데이터를 등록할 수 있어야 합니다.

### 구현할 내용

- Spring Security 적용
- 인증 처리
- Role 기반 권한 처리
- 엔드포인트별 접근 권한 분리

### 권한 예시

| Role | 가능 작업 |
| --- | --- |
| `USER` | 마켓 조회, 자산 조회, 가격 이력 조회, 통계 조회 |
| `ADMIN` | 마켓 생성, 자산 등록, 자산 상태 변경 |
| `SYSTEM` | 가격 등록 |

## 조회 API 확장

각 Chapter에서 생성한 리소스를 확인할 수 있는 기본 조회 API를 먼저 제공합니다.
이후 서비스가 커지면서 목록 조회 조건, 가격 이력 기간 조건, 통계 조회처럼 추가로 필요한 조회 기능을 확장합니다.

### 마켓 목록 조회

`GET /markets`

### 특정 마켓의 자산 목록 조회

`GET /markets/{marketId}/assets`

### 특정 마켓의 자산 단건 조회

`GET /markets/{marketId}/assets/{assetId}`

### 특정 자산의 가격 이력 조회

`GET /markets/{marketId}/assets/{assetId}/prices?from=2026-05-01T00:00:00&to=2026-05-03T23:59:59`

### 요구사항

- 조회 API는 `USER`, `ADMIN`, `SYSTEM` 모두 접근할 수 있습니다.
- 존재하지 않는 마켓이면 `404 Not Found`를 반환합니다.
- 존재하지 않는 자산이거나 마켓과 자산의 관계가 맞지 않으면 `404 Not Found`를 반환합니다.
- 가격 이력 조회는 `from`, `to` 기간 조건에 해당하는 이력을 시간순으로 반환합니다.
- 자산 목록 조회는 이후 필요에 따라 `status`, `symbol`, `currency` 같은 검색 조건을 추가할 수 있습니다.
- 가격 이력 조회는 `from`, `to` 기간 조건을 받을 수 있습니다.
- `from`, `to`가 모두 주어졌다면 `from`은 `to`보다 이전이어야 합니다.
- 날짜 조건이 유효하지 않으면 `400 Bad Request`를 반환합니다.

## Bonus Chapter 1. 공통 응답 포맷을 맞춘다

기능이 늘어나면 응답 구조를 일관되게 관리해야 합니다.

프론트엔드와 외부 연동 클라이언트가 매번 다른 응답 형태를 분기하지 않도록, 바디가 있는 API 응답은 공통 envelope인 `ApiResponse<T>`로 감싸서 반환합니다.

다만 HTTP status가 이미 성공/실패를 표현하므로 `success` 필드는 두지 않습니다.

### 성공 응답 형식

실제 payload는 `data` 필드에 담습니다.

```json
{
  "code": "SUCCESS",
  "message": "Request processed successfully.",
  "data": {
    "id": 1,
    "name": "KOSPI",
    "timezone": "Asia/Seoul"
  }
}
```

목록 조회도 동일하게 `data`에 배열을 담습니다.

```json
{
  "code": "SUCCESS",
  "message": "Request processed successfully.",
  "data": [
    {
      "id": 1,
      "name": "KOSPI",
      "timezone": "Asia/Seoul"
    }
  ]
}
```

생성 API처럼 별도 payload가 없는 성공 응답은 `data`를 `null`로 둡니다.

```json
{
  "code": "SUCCESS",
  "message": "Market created successfully.",
  "data": null
}
```

단, `204 No Content` 응답은 HTTP 의미상 바디를 반환하지 않습니다.

### 실패 응답 형식

실패 응답도 같은 envelope를 사용합니다.

```json
{
  "code": "ASSET_NOT_FOUND",
  "message": "Asset not found.",
  "data": null
}
```

### 요구사항

- 바디가 있는 성공 응답은 `ApiResponse<T>`를 사용합니다.
- 바디가 있는 실패 응답은 `ApiResponse<Nothing>`을 사용합니다.
- 실제 payload는 항상 `data` 필드에 담습니다.
- `success` 필드는 두지 않습니다.
- `code`는 클라이언트가 분기할 수 있는 안정적인 코드입니다.
- `message`는 사람이 읽을 수 있는 설명입니다.
- `204 No Content` 응답은 바디를 반환하지 않습니다.

## Bonus Chapter 2. Redis로 자산별 가격 업데이트를 제어한다

인기 자산에는 여러 시세 제공자와 내부 배치가 동시에 가격을 밀어 넣습니다.

DB의 낙관적 락은 데이터 정합성을 지켜주지만, 충돌이 자주 발생하면 많은 요청이 DB까지 도달한 뒤 실패합니다.
실패한 요청은 재시도되고, 재시도는 다시 DB 부하를 만듭니다.

Kotlin 증권은 이제 가격 업데이트가 DB에 도달하기 전에 자산 단위로 동시 실행을 제어하려고 합니다.

예를 들어 삼성전자 가격 업데이트가 동시에 10개 들어오더라도, 같은 `assetId`에 대한 최신 가격 갱신은 한 번에 하나의 요청만 수행되어야 합니다.
다른 자산의 가격 업데이트는 서로 영향을 주지 않고 병렬로 처리될 수 있어야 합니다.

이를 위해 Redis 기반 분산 락을 도입합니다.

단, Redis 락은 정합성의 유일한 방어선이 아닙니다.
네트워크 지연, TTL 만료, 애플리케이션 장애, 락 해제 실패가 발생할 수 있으므로 Redis 락은 DB 경합을 줄이는 앞단 제어 장치로 사용하고, DB 트랜잭션과 낙관적 락은 최종 정합성 장치로 유지합니다.

### User Story

SYSTEM 권한을 가진 시세 수집기는 여러 외부 제공자로부터 가격 데이터를 수신합니다.

수집기는 다음 API를 호출해 가격을 등록합니다.

`POST /markets/{marketId}/assets/{assetId}/prices`

같은 자산에 대한 가격 등록 요청이 동시에 들어오면, 애플리케이션은 Redis에 자산별 락을 획득한 뒤 가격 이력 저장과 최신 가격 갱신을 처리합니다.

락을 획득한 요청은 정상적으로 DB 트랜잭션을 수행합니다.

락을 획득하지 못한 요청은 짧게 대기한 뒤 실패하며, 최종적으로 `409 CONCURRENCY_ERROR`를 반환합니다.

### 요구사항

- Redis 기반 분산 락을 사용합니다.
- 락 키는 자산 단위로 분리합니다.
- 예: `lock:price-update:asset:{assetId}`
- 현재 모델에서 `assetId`는 `assets.id`이며 전역에서 유일한 기본키입니다.
- 따라서 락 키는 `assetId`만 포함해도 자산 단위로 충돌 범위를 분리할 수 있습니다.
- 같은 자산의 가격 업데이트는 동시에 하나만 처리됩니다.
- 서로 다른 자산의 가격 업데이트는 병렬로 처리될 수 있습니다.
- 락 획득은 atomic 연산으로 수행합니다.
- 예: `SET key value NX PX ttl`
- 락 value에는 요청별 고유 token을 저장합니다.
- Redis 락을 획득한 뒤 DB 트랜잭션을 시작합니다.
- Redis 락 획득 대기는 DB 트랜잭션을 시작하기 전에 수행합니다.
- DB 트랜잭션 안에서는 가격 이력 저장과 최신 가격 갱신을 함께 수행합니다.
- 처리가 끝나면 반드시 락을 해제합니다.
- 락 해제 시에는 token이 일치할 때만 삭제합니다.
- TTL이 만료된 뒤 다른 요청이 같은 락을 획득했을 수 있으므로 단순 `DEL key`는 사용하지 않습니다.
- 예: Lua script로 compare-and-delete를 수행하거나, 동일한 안전성을 제공하는 락 구현체를 사용합니다.
- 락에는 TTL을 둬서 애플리케이션 장애 시에도 영구 락이 남지 않게 합니다.
- TTL은 일반적인 가격 업데이트 처리 시간보다 충분히 길게 설정합니다.
- 처리 시간이 TTL보다 길어질 수 있다면 lease renewal 또는 watchdog 전략을 사용합니다.
- TTL 만료로 Redis 락이 사라져도 DB 낙관적 락이 최종 충돌을 감지해야 합니다.
- 동시 요청 테스트를 작성합니다.

락을 획득하지 못한 경우 기본 정책은 다음과 같습니다.

- 짧은 시간 동안 제한된 횟수만 재시도합니다.
- 재시도 후에도 락을 획득하지 못하면 `409 CONCURRENCY_ERROR`를 반환합니다.
- Redis 락 획득 재시도와 DB 낙관적 락 재시도는 별도 정책으로 관리합니다.
- 즉시 실패 정책을 선택할 수도 있지만, 그 경우 이유를 설명합니다.

Redis 장애 시 기본 정책은 DB 낙관적 락 fallback입니다.

- Redis가 일시적으로 사용할 수 없으면 가격 등록은 DB 트랜잭션과 낙관적 락만으로 처리합니다.
- 이 경우 DB 경합과 충돌이 증가할 수 있으므로 로그, 메트릭, 알림을 남깁니다.
- fail-fast 정책을 선택할 수도 있지만, 가격 데이터 유실 가능성과 운영 영향을 설명합니다.

### 에러 응답 예시

```json
{
  "code": "CONCURRENCY_ERROR",
  "message": "Price update is already in progress for this asset.",
  "data": null
}
```

## Bonus Chapter 3. Redis 락 대기 부하를 Pub/Sub으로 줄인다

Bonus Chapter 2의 Redis 락은 같은 자산에 대한 가격 업데이트가 동시에 실행되지 않도록 막아줍니다.

하지만 락을 획득하지 못한 요청이 짧은 간격으로 계속 재시도하면, 애플리케이션 스레드와 Redis 양쪽에 불필요한 부하가 생깁니다.
특히 인기 자산처럼 동시에 많은 가격 업데이트가 들어오는 경우에는 실패할 가능성이 높은 `SET NX` 요청이 반복해서 Redis로 전달됩니다.

이 문제를 줄이기 위해 락 대기 방식을 단순 spin retry에서 Pub/Sub 알림 기반 대기로 개선합니다.
이 기능의 목적은 락의 정합성을 강화하는 것이 아니라, 동일 자산에 요청이 몰릴 때 발생하는 불필요한 Redis retry와 애플리케이션 대기 비용을 줄이는 것입니다.

락을 가진 요청이 처리를 끝내고 락을 해제하면, 같은 자산의 락을 기다리던 요청들에게 unlock 이벤트를 발행합니다.
대기 중인 요청은 고정 간격으로 계속 재시도하지 않고, unlock 이벤트를 받거나 대기 시간이 만료되었을 때 다시 락 획득을 시도합니다.

단, Pub/Sub은 락의 정합성을 보장하는 장치가 아닙니다.
구독 연결이 끊기거나 메시지가 유실될 수 있으므로, Pub/Sub은 polling 부하를 줄이는 최적화 장치로만 사용합니다.
락 획득은 알림을 받은 뒤에도 반드시 atomic lock 명령으로 다시 시도해야 하며, 최종 정합성은 DB 트랜잭션과 낙관적 락으로 보호합니다.

이 최적화는 실제 운영 지표에서 같은 자산에 대한 락 경합이 반복적으로 확인되고, 제한 재시도만으로 Redis 요청량이나 대기 시간이 과도할 때 도입합니다.
충돌 빈도가 낮거나 즉시 실패 정책으로 충분한 경우, 또는 큐 기반 직렬 처리가 더 단순한 경우에는 Pub/Sub 대기 최적화를 도입하지 않을 수 있습니다.

### User Story

SYSTEM 권한을 가진 시세 수집기가 같은 자산에 대한 가격 등록 요청을 동시에 여러 개 보냅니다.

첫 번째 요청은 Redis 락을 획득하고 가격 이력 저장과 최신 가격 갱신을 수행합니다.

나머지 요청은 같은 자산의 락을 획득하지 못하면, 해당 자산의 unlock 채널을 구독하고 짧은 시간 동안 대기합니다.

락을 가진 요청이 처리를 마치고 락을 해제하면 unlock 이벤트를 발행합니다.

대기 중인 요청은 unlock 이벤트를 받으면 다시 Redis 락 획득을 시도합니다.
락을 획득한 요청만 DB 트랜잭션을 시작할 수 있습니다.

이벤트를 받지 못했거나 전체 대기 예산 안에 락을 획득하지 못하면 기존 정책대로 `409 CONCURRENCY_ERROR`를 반환합니다.

### 요구사항

- Redis 락 획득 실패 시 고정 간격 spin retry만 반복하지 않습니다.
- 락 해제 시 자산 단위 unlock 이벤트를 발행합니다.
- token 검증, 락 삭제, unlock 이벤트 발행은 하나의 원자적 unlock 연산으로 처리합니다.
- 예: Lua script에서 token을 확인한 뒤 `DEL`과 `PUBLISH`를 함께 수행합니다.
- unlock 채널은 자산 단위로 분리합니다.
- 예: `channel:price-lock-released:asset:{assetId}`
- 락 대기 요청은 해당 자산의 unlock 채널을 구독합니다.
- 락 대기 요청은 구독 등록 후 missed event 가능성을 줄이기 위해 즉시 한 번 더 락 획득을 시도합니다.
- 대기 요청은 unlock 이벤트를 받거나 단일 wait timeout이 발생하면 다시 락 획득을 시도합니다.
- 전체 대기 시간 예산을 초과하면 더 이상 대기하지 않고 `409 CONCURRENCY_ERROR`를 반환합니다.
- Pub/Sub 이벤트를 받은 뒤에도 반드시 `SET key value NX PX ttl` 같은 atomic lock 명령으로 락을 다시 획득해야 합니다.
- Pub/Sub 메시지는 유실될 수 있으므로 timeout 기반 재시도를 반드시 유지합니다.
- Pub/Sub은 성능 최적화 장치이며 정합성 보장 장치가 아닙니다.
- Redis 락, token 기반 unlock, TTL, DB 낙관적 락은 계속 유지합니다.
- Redis Pub/Sub 장애가 발생하면 기존 제한 재시도 정책으로 fallback합니다.
- fallback이 발생하면 로그, 메트릭, 알림을 남깁니다.
- 같은 자산에 대한 대기 요청만 같은 unlock 이벤트를 사용합니다.
- 서로 다른 자산의 가격 업데이트와 락 대기는 서로 영향을 주지 않습니다.
- 단일 wait timeout, 전체 대기 시간 예산, 최대 재시도 횟수, retry jitter는 설정으로 관리합니다.
- Pub/Sub 적용 전후로 Redis 락 획득 시도 횟수, 락 대기 시간, `CONCURRENCY_ERROR` 발생률, Redis command throughput, Pub/Sub fallback 횟수, DB optimistic lock conflict 발생률을 비교할 수 있어야 합니다.
- 관측 지표에서 효과가 확인되지 않거나 운영 복잡도가 더 크면 기존 제한 재시도 정책으로 되돌릴 수 있어야 합니다.

### 도입 기준

이 방식은 다음 조건을 만족할 때 선택합니다.

- 같은 `assetId`에 대한 동시 가격 업데이트가 자주 발생합니다.
- 제한 재시도만으로는 Redis `SET NX` 호출량이나 애플리케이션 대기 시간이 과도합니다.
- 요청을 즉시 실패시키면 가격 수집 품질이나 최신성에 영향이 있습니다.
- 큐 기반 직렬 처리로 구조를 바꾸기에는 현재 변경 비용이 큽니다.

다음 상황에서는 Pub/Sub 대기 최적화를 도입하지 않을 수 있습니다.

- 충돌 빈도가 낮아 기존 bounded retry만으로 충분합니다.
- 같은 자산의 가격 업데이트를 큐나 partition 기반 consumer로 직렬 처리할 수 있습니다.
- 최신 가격만 중요해서 중간 업데이트를 병합하거나 덮어써도 됩니다.
- Pub/Sub 구독 리소스 관리 비용이 성능 이득보다 큽니다.

### 설계 주의사항

- unlock 이벤트를 받았다는 사실은 락 획득 성공을 의미하지 않습니다.
- 여러 대기 요청이 같은 unlock 이벤트를 동시에 받을 수 있으므로, 실제로는 그중 하나만 Redis 락 획득에 성공할 수 있습니다.
- 구독 등록 전에 unlock 이벤트가 발행될 수 있으므로 missed event는 timeout 기반 재시도로 회복되어야 합니다.
- unlock 원자 연산에 실패하면 이벤트가 발행되지 않을 수 있으므로 대기 요청은 timeout으로 회복되어야 합니다.
- Pub/Sub 구독을 요청마다 직접 오래 유지하면 커넥션과 스레드 사용량이 커질 수 있으므로, 실제 구현에서는 자산별 대기자 관리와 구독 리소스 관리를 함께 고려합니다.
- 대기 요청이 많아질 경우 unlock 이벤트를 받은 모든 요청이 동시에 Redis로 몰리는 thundering herd 문제가 생길 수 있으므로 jitter 또는 제한된 재시도를 적용할 수 있습니다.

## 에러 코드

| HTTP Status | Code |
| --- | --- |
| 400 | `INVALID_PRICE` |
| 400 | `INVALID_DATE_RANGE` |
| 400 | `INVALID_ASSET_STATUS` |
| 400 | `INVALID_REQUEST` |
| 401 | `UNAUTHORIZED` |
| 403 | `ACCESS_DENIED` |
| 404 | `MARKET_NOT_FOUND` |
| 404 | `ASSET_NOT_FOUND` |
| 409 | `MARKET_ALREADY_EXISTS` |
| 409 | `ASSET_ALREADY_EXISTS` |
| 409 | `CONCURRENCY_ERROR` |
